...
// ─── My Bookings ──────────────────────────────────────────────────────────────
// TEMP_DEBUG: while we're tracking down the field-name mismatch, this skips
// the email filter (so something always shows) and exposes the raw shape of
// the first appointment in a copyable box. Remove the whole TEMP_DEBUG block
// and restore the `mine` filter once we know the real field name.
function MyBookings({ user }) {
  const [bookings, setBookings] = useState([]);
  const [rawSample, setRawSample] = useState(null); // TEMP_DEBUG
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setRawSample(all[0] || "EMPTY_ARRAY — backend returned zero appointments total"); // TEMP_DEBUG
        // TEMP_DEBUG: filter disabled — showing everything the backend
        // returns so the page isn't empty while we diagnose field names.
        setBookings(all);
      } catch (err) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* TEMP_DEBUG — delete this block once the field-name issue is fixed */}
      {!loading && rawSample && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
          <p className="text-xs font-semibold text-yellow-800 mb-2">
            🔧 Debug: copy everything below and paste it back into the chat
          </p>
          <pre className="text-[11px] whitespace-pre-wrap break-all bg-white border border-yellow-100 rounded-lg p-3 max-h-60 overflow-auto">
            {typeof rawSample === "string" ? rawSample : JSON.stringify(rawSample, null, 2)}
          </pre>
        </div>
      )}
